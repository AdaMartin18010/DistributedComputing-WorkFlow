import React, { useState } from 'react'
import { Card, Button, message, Space, Tabs, Tag } from 'antd'
import { PlayCircleOutlined, CheckCircleOutlined, CloseCircleOutlined } from '@ant-design/icons'
import Editor from '@monaco-editor/react'
import { tlaApi } from '../services/api'

const { TabPane } = Tabs

function TLAVerifier() {
  const [model, setModel] = useState(`EXTENDS Naturals

VARIABLES x, y

Init == x = 0 /\\ y = 0

Next == \\/ x' = x + 1
         \\/ y' = y + 1

Spec == Init /\\ [][Next]_<<x, y>>

THEOREM Spec => []TypeOK`)
  
  const [properties, setProperties] = useState('')
  const [verifying, setVerifying] = useState(false)
  const [result, setResult] = useState(null)

  const handleVerify = async () => {
    try {
      setVerifying(true)
      setResult(null)
      const data = await tlaApi.verify({
        model,
        properties: properties ? properties.split('\n').filter(p => p.trim()) : null,
      })
      setResult(data)
      if (data.status === 'success') {
        message.success('验证成功')
      } else if (data.status === 'error') {
        message.error('验证失败: ' + data.error)
      }
    } catch (error) {
      message.error('验证失败: ' + error.message)
    } finally {
      setVerifying(false)
    }
  }

  return (
    <Card title="TLA+模型验证">
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        <div>
          <Button
            type="primary"
            icon={<PlayCircleOutlined />}
            onClick={handleVerify}
            loading={verifying}
            size="large"
          >
            开始验证
          </Button>
        </div>

        <Tabs defaultActiveKey="model">
          <TabPane tab="TLA+模型" key="model">
            <Editor
              height="400px"
              defaultLanguage="tla"
              value={model}
              onChange={setModel}
              theme="vs-dark"
              options={{
                minimap: { enabled: false },
                fontSize: 14,
                wordWrap: 'on',
              }}
            />
          </TabPane>
          <TabPane tab="属性" key="properties">
            <Editor
              height="400px"
              defaultLanguage="tla"
              value={properties}
              onChange={setProperties}
              theme="vs-dark"
              placeholder="输入要验证的属性（每行一个）"
              options={{
                minimap: { enabled: false },
                fontSize: 14,
                wordWrap: 'on',
              }}
            />
          </TabPane>
        </Tabs>

        {result && (
          <Card title="验证结果" size="small">
            <div>
              <Space>
                <Tag color={result.status === 'success' ? 'green' : result.status === 'error' ? 'red' : 'orange'}>
                  {result.status === 'success' && <CheckCircleOutlined />}
                  {result.status === 'error' && <CloseCircleOutlined />}
                  {result.status}
                </Tag>
              </Space>
            </div>
            {result.error && (
              <div style={{ marginTop: '16px', color: 'red' }}>
                <strong>错误:</strong> {result.error}
              </div>
            )}
            {result.counter_example && (
              <div style={{ marginTop: '16px' }}>
                <strong>反例:</strong>
                <pre style={{ background: '#f5f5f5', padding: '12px', borderRadius: '4px' }}>
                  {JSON.stringify(result.counter_example, null, 2)}
                </pre>
              </div>
            )}
            {result.result && (
              <div style={{ marginTop: '16px' }}>
                <strong>结果:</strong>
                <pre style={{ background: '#f5f5f5', padding: '12px', borderRadius: '4px' }}>
                  {JSON.stringify(result.result, null, 2)}
                </pre>
              </div>
            )}
          </Card>
        )}
      </Space>
    </Card>
  )
}

export default TLAVerifier
